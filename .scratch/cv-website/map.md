...
- [Prototype the Site's visual design language](issues/10-prototype-visual-design.md): **C9, photographic-editorial**, built from `nature-conservation-hero-01` + `wooden-cabin-booking` in Tom's curated collection. Full-bleed photographic hero with the wordmark rotated up the left margin in salmon-terracotta, a split experience band, Bodoni Moda display over Inter with the optical-size axis pinned per role (both self-hosted from `public/fonts/`, so the Site makes no external request — see `docs/webfonts.md`), scroll-triggered fade-in as progressive enhancement, and an Elsewhere links band. Now the Site. Notable constraints recorded in the ticket: the `font-variation-settings` are load-bearing, `.cv-nav`'s z-index prevents a real hit-testing bug, the CTA's container query threshold is measured, and any `!important` in a component style loses to Tailwind preflight's layered `!important`.

- [Optimise the Site's imagery](issues/11-optimise-site-imagery.md) (open): the two photographs ship as full-resolution Unsplash originals in `public/`, 3.9 MB of image payload and a 4.3 MB `cv.pdf`. Measured with sharp: most of the win is **resizing** to the ~1900px the design actually displays (~80% on its own), and **WebP is not a win over mozjpeg** at matched dimensions (763 KB vs 710 KB) — AVIF is, at 440 KB. Recommendation is `src/assets/` + `astro:assets` rather than hand-converting, since the mechanism carries over to the real photography.

## Not yet specified

- Typography and the visual design language: **decided and implemented.** [#10](issues/10-prototype-visual-design.md) ran eleven variants across seven rounds; **C9 (photographic-editorial) won and is now the Site** — folded into `cv-page.astro`, `global.css`, new `src/data/links.ts` and `src/data/photos.ts`, and `scripts/generate-pdf.ts`. #08's four-colour palette is retired: C9 commits to a salmon-terracotta accent and drops the pale sage, which the reference collection gave no support for. Dark mode was never built and is still unspecified.
- `client` casing in the CV data: `Outokumpu Stainless AB` vs `Outokumpu stainless AB` across three project files, so one client renders as two in any design that groups by client (surfaced by #10's C3). Tom's call which casing is right; also worth deciding whether `client` should be a controlled value in the collection schema rather than free text.
